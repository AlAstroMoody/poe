...
export interface WasmData {
  ...
  GetPassiveSkillByIndex: (
    ...
    | { Index: number; PassiveSkillGraphID: number; StatsKeys?: number[] }
    | undefined;
  ...
  TimelessJewelConquerors: Record<
    number,
    Record<string, { Index: number; Version: number } | undefined> | undefined
  >;
  TimelessJewelSeedRanges: Record<
    number,
...
function initializeFromGlobal(): void {
  const go = (typeof window !== "undefined" &&
    (window as unknown as { Go?: unknown }).Go) as
  ...
  const tj = (
    globalThis as unknown as {
      go?: {
        "timeless-jewels"?: { data: WasmData; calculator: WasmCalculator };
      };
    }
  ).go?.["timeless-jewels"];
  if (!tj?.data || !tj?.calculator)
    throw new Error("timeless-jewels WASM module not found on globalThis.go");
  wasmData = tj.data;
...
const WASM_FETCH_OPTIONS: RequestInit = import.meta.env.DEV
  ? { cache: "no-store" }
  : {};
...
async function fetchWasmArrayBuffer(
  ...
  const res = await fetch(url, WASM_FETCH_OPTIONS);
...
export async function loadWasm(
  ...
  try {
    instance = await instantiateWasmFromUrl(
      PRIMARY_WASM_URL,
      "primary",
      go,
      onProgress,
